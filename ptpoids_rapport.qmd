---
title: "Rapport statistique v2.0"
cache: true
---
...
```{r}
#| label: corr1
#| fig-cap: Corrélations - Inclusion
...
```{r}
#| label: corr2
#| fig-cap: Corrélations - Questionnaire
...
inclus |> 
dplyr::select(16:21,23:31) |> 
  dplyr::select(-ends_with("dte")) |> 
tbl_summary(missing = "no",
            value = list(domic_on = "yes",
                         consultped_on = "yes",
                         emploi_on = "yes")) |> 
modify_header(label ~ " ") |> 
bold_labels() |> 
...
bold_labels() |> 
add_n() |> 
  pexptabph(nomfich = classeur, nomsheet = "stress1", 
  lg = TRUE,
  exp = expx)
```
...
bold_labels() |> 
add_n() |> 
  pexptabph(nomfich = classeur, nomsheet = "med_poids", 
            lg = TRUE,
            exp = expx) |> 
...
Pour la comparaison entre les Z-scores (poids, taille & périmètre crânien) & les trois consultations un test exact de \text{Fisher} a été utilisé.
