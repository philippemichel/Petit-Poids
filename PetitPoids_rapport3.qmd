...
```{r}
#| label: mois
## Recodage de zscore$mois
zscore$mois <- zscore$mois %>%
  as.character() %>%
  fct_recode(
    "mois 2" = "2",
    "mois 4" = "4",
...
## Réordonnancement de zscore$mois
zscore$mois <- zscore$mois %>%
  fct_relevel(
...
quest |> 
dplyr::select(parent, 8, 11, 12,14, 33) |> 
tbl_summary(missing = "no", 
            by = parent,
            statistic = list(all_categorical() ~ "{n}/{N} ({p})")) |> 
modify_header(label ~ " ") |> 
...
nom <- c("pp",
         "Le stress des pesées",
         "Se réveiller la nuit, la fatigue",
"Les difficultés d’alimentation",
"Ne pas avoir d’entourage familial",
"Rien, tout s’est bien passé",
"Autre")

zz <- quest |> 
dplyr::select(parent, 54:59) |>
mutate_all(~as.factor(replace_na(as.character(.), "no")))
var_label(zz) <- nom
zz |> 
tbl_summary(missing = "no",
by = parent,
...
Les comparaisons entre données catégorielles ont été faites par un test exact de Fisher.  Les comparaison des z-scores aux différentes visites ont été calculées en analyse variance (ANOVA)
