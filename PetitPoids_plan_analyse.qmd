---
title: "Plan d'analyse statistique V2.0"
toc: false
lot: false
lof: false
cache: true
...
\begin{center}
\url{https://github.com/philippemichel/Petit-Poids}
\end{center}
...
Il s'agit d'une étude observationnelle simple pour lesquelles il est difficile de préciser un nombre de cas nécessaires en l'absence de test statistique. Néanmoins en utilisant un calcul de taille pour des enquêtes avec une réponse à \qty{50}{\percent} pour le critère principal (cas le plus défavorable) & une erreur admissible de \qty{15}{\percent}, dans une grande population, on peut estimer le nombre de cas nécessaires à `r zz`. 

## Qualité des données

Une analyse de corrélation (package `corr` [@corr]) entre toutes les variables (un tableau pour les données démographique, un pour les questionnaires parents & un pour les questionnaires médecins) sera réalisée & présentée sur un graphique de corrélation. Si certaines variables se montrent anormalement corrélées elles devront être exclues de l'étude après accord du promoteur.

Un décompte des données manquantes sera réalisé & présenté par un tableau ou un graphique. Les variables comportant trop de données manquantes ou non utilisables ne seront pas prises en compte après validation par le promoteur. Les cas où la ré-hospitalisation de l’enfant (critère principal) n'est pas noté seront exclus. 
...
*Observer le devenir à court terme (ré-hospitalisation) des nouveau-nés ayant quitté le service de néonatologie de l’hôpital \textsc{novo} (site de Pontoise) entre 2012 et 2022 avec un poids inférieur à 2 kilos.*

Le nombre d'enfants ayant été ré-hospitalisés durant le mois après sortie de l'hôpital sera présenté en nombre & pourcentage avec son intervalle de confiance. 
...
Les courbes d'évolution (données brutes & Z-score) seront tracées.
...
Ces critères purement descriptifs issus du questionnaire *médecins* reprennent les résultats présentés dans les tableaux descriptifs. Un eprésentation reprenant les données numérique sera réalisée ainsi q'ine regroupant es réponses en :

- moins de \qty{2000}{g}
- \qty{2000}{g}
- plus de \qty{2000}{g}

Des graphiques pourront être tracés pour présenter ces résultats plus spécifiquement.
