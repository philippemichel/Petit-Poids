...
Vu le faible nombre de cas on ne fera pas d'hypothèse de normalité & les variables numériques seront présentées par leur médiane avec les quartiles & comparées par le test non paramétrique de Wilcoxon. Les variables discrètes seront présentés en nombre avec le pourcentage & comparées par le test du \textchi$^2$ de Spearman sous réserve d'un effectif suffisant. À défaut le test exact de Fischer sera utilisé. Des graphiques seront réalisés pour les résultats importants (package `ggplot2` [@ggplot]).
...
Le critère principal est : *Observer le devenir à court terme (réhosptalisation) des nouveau-nés ayant quitté le service de néonatologie de l’hôpital NOVO (site de Pontoise) entre 2012 et 2022 avec un poids inférieur à 2 kilos.* 
...
La médiane avec les quartiles sera présentée pour chacune des trois mesures pour chaque consultation. Une représentation graphique (diagramme en box-plot ou violon-plot) sera réalisée ainsi que des graphiques d'évolution avec les droites de régression \marginpar{\small Pour la visite à onze mois il risque d'y avoir de nombreuses données manquantes ce qui limiterait la valeur des régressions.}. Vu le faible nombre de cas un diagramme *en fagots* pourra être tracé.

Ces mesures seront considérées comme *normales* si comprises entre -2 & 2 DS. Le nombre & le pourcentage de mesures normales seront présentés pour chaque consultation. Des diagrammes de flux pour chaque mesure pourront être dessinés.  
